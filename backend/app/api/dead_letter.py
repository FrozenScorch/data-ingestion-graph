...
async def retry_dlq_item(
    ...
    try:
        ...
        # Retrieve the parent run and node config for proper retry
        from app.models.execution import Run
        from sqlalchemy import select as _s
        ...
        run_node = await run_node_with_retry(
            ...
            input_data=item.input_data,
            state={},
            max_retries=1,  # Single attempt for DLQ retry
        )

        node_result_success = run_node.status == "completed"

        if node_result_success:
