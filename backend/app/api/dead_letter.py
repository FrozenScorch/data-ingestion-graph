...
async def _check_dlq_item_access(item, current_user, db):
    if not item.run_id:
        if current_user["role"] == "admin":
            return
        # A runless legacy item cannot be attributed to a tenant. Conceal it
        # from non-admin users even if they know an old item identifier.
...
async def list_dlq_items(
    ...
    if current_user["role"] != "admin":
        ...
        query = (
            query.join(Run, DeadLetterQueue.run_id == Run.id)
            .join(Graph, Run.graph_id == Graph.id)
            .where(owner_filter)
        )
        count_query = (
            count_query.join(Run, DeadLetterQueue.run_id == Run.id)
            .join(Graph, Run.graph_id == Graph.id)
            .where(owner_filter)
        )
    ...
    try:
        ...
        run_node = await run_node_with_retry(
            ...
            state={
                "owner_id": str(
                    (
                        await db.execute(_s(Graph.owner_id).where(Graph.id == run.graph_id))
                    ).scalar_one()
                )
            },
            max_retries=1,
