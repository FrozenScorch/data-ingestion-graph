...
export const openrouterService = {
  ...
  async listModels(category?: 'chat' | 'embedding'): Promise<ModelListResponse> {
    ...
    return api.get<ModelListResponse>('/api/openrouter/models', params);
  },

  /** List embedding models only */
  async listEmbeddingModels(): Promise<ModelListResponse> {
    return api.get<ModelListResponse>('/api/openrouter/embedding-models');
  },
  ...
  async estimateCost(
    ...
    return api.get<CostEstimate>('/api/openrouter/cost-estimate', {
      model,
