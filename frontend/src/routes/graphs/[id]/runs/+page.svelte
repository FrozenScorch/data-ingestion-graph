...
<div class="p-6 space-y-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <h1 class="text-xl font-bold text-gray-100 mt-1">Run History</h1>
    </div>
    <button
  ...
  {:else if execution.runs.length === 0}
    <div class="text-sm text-gray-600 py-8 text-center">No runs yet</div>
  {:else}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-800">
            ...
            <th class="text-right px-4 py-3 text-xs text-gray-500 font-medium"></th>
          </tr>
        </thead>
        <tbody>
          {#each execution.runs as run (run.id)}
            <tr class="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
              ...
              <td class="px-4 py-3">
                <a href="/graphs/{graphId}/runs/{run.id}" class="text-gray-300 hover:text-white transition-colors font-mono text-xs">{run.id.slice(0, 8)}...</a>
              </td>
              ...
              <td class="px-4 py-3 text-right">
                <a href="/graphs/{graphId}/runs/{run.id}" class="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">View</a>
              </td>
            </tr>
          {/each}
        </tbody>
